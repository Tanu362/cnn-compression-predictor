...
def download_baseline():
    if not os.path.exists("baseline.pth"):      # only download if not already there
        file_id = "1wxux74QrYPjTTJ7QZ3QElmKFhm2nEerc"      # ← paste YOUR file ID here
        url = f"https://drive.google.com/uc?id={file_id}"
        gdown.download(url, "baseline.pth", quiet=False)

download_baseline()   # this runs automatically when app starts
...
def load_baseline_model():
    """Load the ResNet18 baseline checkpoint."""
...
def load_cifar10_test():
    """Download CIFAR-10 test set and return as list of (PIL image, label)."""
...
def build_compressed_model(baseline_model, pruning, quant, svd):
    """
    Apply selected compression techniques in order:
    Prune → SVD → Quantize
    (No fine-tuning at inference time — we use pre-trained baseline)
    """
...
def classify_image(model, img_tensor, quant_mode):
    ...
    inp  = norm(img_tensor).unsqueeze(0)   # (1, C, H, W)

    if quant_mode == "INT8":
        model = model.cpu()
        inp   = inp.cpu().contiguous()
    elif quant_mode == "FP16":
        model = model.cpu()   # safer for half on CPU
        inp   = inp.half().cpu()
    else:
        model = model.cpu()
        inp   = inp.cpu()
...
def predict_metrics(pred_models, pruning, quant, svd):
    ...
    return {
        ...
        "gflops":         BASELINE["gflops"],   # GFLOPs constant (Conv2d dominated)
    }
...
def main():
    ...
    random_btn = st.sidebar.button("🎲 Pick Random Image")
    ...
    with tab2:
        ...
        img_tensor, true_label = dataset[img_index]   # (C,H,W) tensor, int

        # Upscale for display: 32×32 → 224×224
        ...
        with col_img:
            st.image(img_large,
                     caption=f"Image #{img_index} | True label: {CIFAR10_CLASSES[true_label]}",
                     width=224)

        with col_result:
            if run_btn or not active:
                with st.spinner("Building compressed model and classifying..."):
                    try:
                        ...
                        if is_correct:
                            st.success(f"✅ Predicted: **{pred_label}** ({conf:.1f}% confidence)")
                        else:
                            st.error(
                                f"❌ Predicted: **{pred_label}** ({conf:.1f}% confidence) "
                        ...
                        bar_colors = ["#2ecc71" if c == CIFAR10_CLASSES[true_label]
                        ...
                        st.info("Note: INT8 quantization requires baseline.pth to be loaded "
                                "and may be slow as it runs on CPU.")
            else:
                st.info("👈 Press **Run Prediction** to classify the image with "
                        "the compressed model.")

        st.divider()

        # Show 10 random samples from the test set
    ...
    with tab3:
        ...
        st.dataframe(
            filtered[["method", "level", "accuracy_pct", "nonzero_params",
        ...
        for method, grp in df_results.groupby("method"):
            ax3.scatter(range(len(grp)), grp["accuracy_pct"],
